...
def add_text_with_italics(paragraph, text):
    """
    Converts underscore-wrapped text to italic Word runs.
    Example: 'In the _beginning_ God' → beginning is italic.
    """
...
with open(INPUT_FILE, 'r', encoding='utf-8') as f:
    for raw in f:
        ...
        # Remove underscore-only spacer lines
        ...
        if book_title:
            p = doc.add_paragraph()
            add_text_with_italics(p, book_title.group(1))
            for run in p.runs:
                run.bold = True
                run.font.size = Pt(22)
