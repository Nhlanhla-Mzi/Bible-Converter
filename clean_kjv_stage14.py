...
with open(INPUT_FILE, 'r', encoding='utf-8') as f:
    for raw in f:
        ...
        # -------- DASHED BOOK TITLES → FULL KJV TITLES
        ...
        book_chap = BOOK_CHAPTER_LINE.match(line)
        if book_chap:
            pending_chapter = book_chap.group(2)
            continue

        # -------- Verses
        verse = VERSE_LINE.match(line)
        if verse:
            verse_num, spacer, verse_text = verse.groups()
            p = doc.add_paragraph()
